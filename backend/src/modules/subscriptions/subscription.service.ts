...
import { calculateMonthlyAmountCents, TRIAL_PERIOD_DAYS, DEFAULT_CURRENCY, type SubscriptionPlan } from "./pricing.js";
...
export async function startTrial(businessId: string, plan: SubscriptionPlan) {
  const [existing] = await db.select().from(subscriptions).where(eq(subscriptions.businessId, businessId));
  if (existing) {
...
export async function startCardBinding(businessId: string, returnUrl: string) {
  ...
  if (subscription.status !== "trial") {
    throw new SubscriptionError("Привязка карты доступна только во время пробного периода", "invalid_state");
  }
...
export async function chargeSubscription(subscriptionId: string, promoCode?: string) {
  ...
  const baseAmountCents = calculateMonthlyAmountCents(subscription.plan as SubscriptionPlan, staffCount);
  ...
  const previousCharges = await db.select().from(payments).where(eq(payments.subscriptionId, subscriptionId));
  const isFirstCharge = !previousCharges.some((p) => p.purpose === "recurring" && p.status === "succeeded");
  ...
  const { payment, idempotenceKey } = await chargeRecurringPayment({
    ...
    metadata: { businessId: subscription.businessId, subscriptionId: subscription.id, purpose: "recurring" },
  });
